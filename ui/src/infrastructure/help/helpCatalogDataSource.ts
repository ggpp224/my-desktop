...
export function getHelpCommands(): HelpCommandItem[] {
  return [
    ...
    },
    {
      ...
      description:
        '按固定 JQL 查询；我的任务为经办人或开发人员是当前用户、全量未完成（不限迭代，不含缺陷）；经办人bug 为类型为缺陷且未解决，按修复版本升序；经办人任务同经办人bug但类型含任务/子任务/缺陷；待办bug 按当前日期落入的修复版本迭代、状态打开；处理中bug 同待办但状态为处理中；待办/处理中列表顶可切换前一/当前/下一迭代',
    },
    {
      section: 'Cursor',
