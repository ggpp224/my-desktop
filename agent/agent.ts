...
工作流：开始工作/执行 start-work → run_workflow(name=start-work)。standalone → run_workflow(name=standalone)。启动 cpxy/react18/base18/scm/cc-web/biz-solution/uikit/shared → run_workflow_step(workflow=start-work 或 standalone，taskKey=对应 key)。升级集测react18的nova版本 → run_workflow(name=upgrade-react18-nova)。升级集测cc-web的nova版本 → run_workflow(name=upgrade-cc-web-nova)。
...
浏览器：打开 Jenkins/URL → open_browser(url=完整 URL)。打开集测环境 → open_jice_env()。打开测试环境 → open_test_env()。打开json配置中心 → open_json_config_center()。打开某项目 Jenkins 任务页 → open_jenkins_job(job=nova|cc-web|react18|base|base18|biz-solution|biz-guide|scm)。Shell：执行命令 → run_shell(command=命令)。`;
