...
浏览器：打开 Jenkins/URL → open_browser(url=完整 URL)。打开集测环境 → open_jice_env()。打开测试环境 → open_test_env()。打开某项目 Jenkins 任务页 → open_jenkins_job(job=nova|cc-web|react18|base|base18|biz-solution|biz-guide|scm)。Shell：执行命令 → run_shell(command=命令)。`;
